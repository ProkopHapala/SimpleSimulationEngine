...
def _wrap_ptr(name: str, length: int, stride: int = 1, shape=None) -> np.ndarray | None:
    ...
    if shape is None:
        if stride > 1:
            ...
            shape = (size,)
    arr = np.ctypeslib.as_array(ptr, shape=(shape[0] * shape[1] if len(shape) == 2 else shape[0],))
...
def inertial_transform(pos_shift: np.ndarray | None, vel_shift: np.ndarray | None):
    pos_ptr = None if pos_shift is None else np.ascontiguousarray(pos_shift, dtype=np.double).ctypes.data_as(c_double_p)
    vel_ptr = None if vel_shift is None else np.ascontiguousarray(vel_shift, dtype=np.double).ctypes.data_as(c_double_p)
...
def setup_jupiter_moons_skirmish():
    ...
    add_planet("Ganymede", 1.48190e24, 2.6312e6, (1.07041e9, rJup, 0.0), (vJup, 10880.0, 0.0))
    add_planet("Calisto", 1.07590e24, 2.4103e6, (1.88271e9, rJup, 0.0), (vJup, 8204.0, 0.0))
...
def get_ship_trj_buffer(index: int, name: str, kind: str = "trjPos") -> np.ndarray | None:
    return get_buffer(f"ship[{index}]:{name}.{kind}")


def get_planet_trj_buffer(index: int, name: str, kind: str = "trjPos") -> np.ndarray | None:
