...
/*
    ...
    this (F...)(F components){ foreach(i, v; components) static if (i < size) arrayof[i] = cast(T)v; }
    this (S)(S str) if (isSomeString!S){ arrayof = parse!(T[size])(str); }
*/
...
/*
    ...
    Vector!(T,size) opUnary(string op)( Vector!(T,size) v1, Vector!(T,size) v2 ) const body{
        Vector!(T,size) res;
        res.opAssign!op( v1, v2 );
        return res;
    }

T dot(T, int size) (Vector!(T,size) a, Vector!(T,size) b)body{
