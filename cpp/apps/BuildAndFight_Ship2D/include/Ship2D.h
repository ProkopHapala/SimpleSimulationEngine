...
class Yacht2D : public RigidBody2D {
	...
	inline void applySailForces( const Vec2d& windSpeed, const Vec2d& watterSpeed ){
		...
		printf( " keel   "); keel  .assertAeroForce( *this, lvel, 1000.0 );
		printf( " rudder "); rudder.assertAeroForce( *this, lvel, 1000.0 );
		lvel.set_sub( windSpeed, vel   );
		printf( " mast   "); mast.assertAeroForce  ( *this, lvel, 1.225  );
