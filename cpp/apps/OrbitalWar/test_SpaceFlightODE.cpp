...
double  uT    = 1.0; // duration of each CP interval
double inv_uT = 1.0/uT;

// rocket parameters - Saturn V
double  vexhaust      = 4500.0; // [km/s]
double  dm_F          = 1.0/vexhaust;
double  mass_initial  = 2.970e+6    *0.0; // put there zero to switch off the engine
double  mass_empty    = 130e+3 + 140e+3 + 40e+3 + 13.5e+3;
double  thrust_full   = 35e+6;

// planet parameters
const double planet_R   = 6371e+3;
double       planet_GM  = 3.9860044189+14; //https://en.wikipedia.org/wiki/Standard_gravitational_parameter
...
inline double getSpline( double u, double* CPs ){
    ...
    double cp2 = CPs[2];
    double cp3 = CPs[3];
    return Spline_Hermite::val( u, cp1, cp2, (cp2-cp0)*0.5d, (cp2-cp1)*0.5d );
}
...
inline Vec3d getSpline3d( double u, Vec3d* CPs ){
    ...
    return { // TODO Optimize this
        Spline_Hermite::val( u, cp1.x, cp2.x, (cp2.x-cp0.x)*0.5d, (cp2.x-cp1.x)*0.5d ),
        Spline_Hermite::val( u, cp1.y, cp2.y, (cp2.y-cp0.y)*0.5d, (cp2.y-cp1.y)*0.5d ),
        Spline_Hermite::val( u, cp1.z, cp2.z, (cp2.z-cp0.z)*0.5d, (cp2.z-cp1.z)*0.5d )
    };
}
...
void getAcceleration( double t, int n, double * Ys, double * dYs ){
    ...
    if( m > mass_empty ){   //  check if propelant exhausted
        ...
        Vec3d  dir    = getSpline3d( u, dirCPs    );
        double thrust = getSpline  ( u, thrustCPs );
...
void TestApp_SpaceFlightODE::draw(){
	...
	printf( " %g : (%g,%g,%g) %g \n ", odeint.t, pos->x,pos->y,pos->z, *mass );
