...
void truss2ocl( const SpaceCraft& craft, const Truss& truss, OCL_Orb& ocl ){
    ...
    for(int i=0; i<truss.edges.size(); i++){
        const TrussEdge& e = truss.edges[i];
        nneighs[e.a]++;
        nneighs[e.b]++;
    }
    int nneighmax = 0;
    for(int i=0; i<truss.points.size(); i++){ int ni=nneighs[i]; if(ni>nneighmax)nneighmax=ni; }
    printf( "nneighmax %i \n", nneighmax );

    ocl.initBuffs_Orb( truss.points.size(), nneighmax, true );
    ...
    for(int i=0; i<truss.edges.size(); i++){
        const TrussEdge& e = truss.edges[i];
        int i0 = nneighs[e.a];
        int ia = e.a*ocl.nNeighMax + nneighs[e.a];
        int ib = e.b*ocl.nNeighMax + nneighs[e.b];
        ocl.neighs[ ia ] = e.b; 
        ocl.neighs[ ib ] = e.a;
