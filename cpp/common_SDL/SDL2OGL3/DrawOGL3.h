...
GLMesh* glQuadGrid( Vec2i ns ){
    ...
    for(int iy=0; iy<ns.y; iy++){
        ...
        for(int ix=0; ix<ns.x; ix++){
            inds[i  ].set( iy1+ix, iy2+ix,   iy2+ix+1 );
            inds[i+1].set( iy1+ix, iy1+ix+1, iy2+ix+1 );
            i+=2;
        }
    }
    GLMesh* mesh=new GLMesh();
    mesh->init( nverts, nquads*3*2, (GLuint*)inds, (GLfloat*)verts, NULL, NULL, NULL);
...
GLMesh* glTriangleGrid( int n ){
    ...
    for(int iy=0; iy<n; iy++){
        ...
        for(int ix=0; ix<nx; ix++){
            inds[i].set( i1+ix, i1+ix+1, i2+ix ); i++;
            if(ix==(nx-1))continue;
            inds[i].set( i1+ix+1, i2+ix, i2+ix+1 ); i++;
        }
        i1 = i2;
    }
    //printf( "i %i \n", i, nquads );
    GLMesh* mesh=new GLMesh();
    mesh->init( nverts, nquads*3, (GLuint*)inds, (GLfloat*)verts, NULL, NULL, NULL);
    mesh->draw_mode = GL_TRIANGLES;
    delete [] verts;
    delete [] inds;
    return mesh;
...
GLMesh* glHalfHexGrid( Vec2i ns ){
    int nverts = (ns.y+1)*(ns.x+1+ns.y);
    int nquads = ns.y*(ns.x*2+ns.y);
    float dy = 1.0/ns.y;
    ...
    for(int iy=0; iy<ns.y; iy++){
        ...
        int i2 = i1+(nx+1);
        for(int ix=0; ix<=nx; ix++){
            inds[i].set( i1+ix, i2+ix, i2+ix+1 ); i++;
            if(ix==(nx))continue;
            inds[i].set( i1+ix, i1+ix+1, i2+ix+1 ); i++;
        }
        i1 = i2;
    }
    printf( "i %i \n", i, nquads );
