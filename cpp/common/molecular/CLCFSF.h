...
class CLCFSF{
    ...
    // ----- Integral Tables
    double* ISs = 0;  // Overlap  = < chi_i(r) |    1       | chi_j(r) >  Overlap interactin between two  spherical bais functions
    double* ITs = 0;  // Kinetic  = < chi_i(r) | Laplace    | chi_j(r) >  Kinetic energy interactin between two spherical functions
    double* IKs = 0;  // Coulomb  = < chi_i(ri) | 1/|ri-rj| | chi_i(rj) >
    ...
    void realloc( int natom_, int nOrb_, int perOrb_, int nsamp_, int natypes_ ){
        ...
        if(nsamp != nsamp_){
            ...
            _realloc( ISs, nsampMem );
            _realloc( ITs, nsampMem );
            _realloc( IKs, nsampMem );
            for(int i=0;i<natypes;i++){ if(IrhoV[i])delete [] IrhoV[i]; }
            if( natypes != natypes_ ){ natypes = natypes_; _realloc( IrhoV, natypes ); }
            for(int i=0;i<natypes;i++){ if(IrhoV[i]) IrhoV[i]=new double[nsampMem]; }
        }
    }

    inline void setR( double Rcut_ ){
        Rcut  = Rcut_;
        dsamp = Rcut/nsamp;
    }

    inline int getOrbOffset(int iorb){ return iorb*nOrb;  }
    inline int getRhoOffset(int iorb){ return iorb*nqOrb; }

    inline bool checkOrbitalCutoff(int i, int j){
    ...
    */
    ...
    inline double evalPauli( int io, int jo ){
        // Pauli Energy between two orbitals depend on its overlap  ~S^2
    ...
    double projectOrbs(){   // project density of all orbitals onto axuliary charge representation ( charges, dipoles and axuliary functions )
        ...
        for(int io=0; io<nOrb; io++){
            ...
            eQs[io] = projectOrb(io, coefs+i0, erhos+irho0, erhoPs+irho0, edips[io] );
            irho0+=nqOrb; i0+=perOrb;
        }
    }

    void electrostaticsOrbPair( int nrho, int nV, double* rhos, double* rhoPs, double* rhoFs,  double* Vs, double* VPs, double* VFs ){
        for(int i=0; i<nV; i++){
            Vec3d  pi  = rhoPs[i];
            double qi  = rhos [i];
            for(int j=0; j<nrho; j++){
                ...
                double x   =  r - i;
                double qj  = rhos [j];
                Spline_Hermite::valdval( x, S, dS, ICs[i], ICs[i+1], ICs[i+2], ICs[i+3] );  // coulomb integrals
            }
        }
    }
        ...
        for(int io=0; io<nOrb; io++){
            ...
            for(int jo=0; jo<nOrb; jo++){
                ...
                //if( checkOrbitalCutoff( i, j ) ){
                    ...
                    electrostaticsOrbPair( nrho, nV, erhos+i0, erhoPs+i0, erhoFs+i0,  erhos+j0, erhoPs+j0, erhoFs+j0 );
                }else{
                    double ir2 = 1/(r2+r2safe);
                    double ir  = sqrt(ir2);
                    // Calculate approximate long-range electrostatics (using charges and dipoles)
                    const Vec3d& dipj = odips[j];
                    E = ir +                                   // charge-charge
