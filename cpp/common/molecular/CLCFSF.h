...
class CLCFSF{ public:
    ...
    double* ICs = 0;  // Coulomb  = < chi_i(ri) | 1/|ri-rj| | chi_i(rj) >
    ...
    void realloc( int natom_, int nOrb_, int perOrb_, int nsamp_, int natypes_ ){
        ...
        if(nsamp != nsamp_){
            ...
            _realloc( ISs , nsampIMem );
            _realloc( ITs , nsampIMem );
            _realloc( ICs , nsampIMem );
            for(int i=0;i<natypes;i++){
                if(IrhoV[i])delete [] IrhoV[i];
                if(PPs  [i])delete [] PPs  [i];
            }
            if( natypes != natypes_ ){
                natypes = natypes_;
                _realloc( IrhoV, natypes );
                _realloc( PPs  , natypes );
            }
            for(int i=0;i<natypes;i++){
                if(PPs  [i]) PPs  [i]=new double[nsampMem ];
                if(IrhoV[i]) IrhoV[i]=new double[nsampIMem];
            }
        }
    }
    ...
    */
        ...
        for(int i=i0; i<i0+perOrb; i++){
            ...
            *erhoPs=pi;  erhoPs++;
            ...
            for(int j=i0; j<i; j++){
                ...
                double x   =  r - i;
                //Spline_Hermite::valdval( x, S, dS, ISs[i], ISs[i+1], ISs[i+2], ISs[i+3] ); // Overlap
                //Spline_Hermite::valdval( x, K, dK, ITs[i], ITs[i+1], ITs[i+2], ITs[i+3] ); // Kinetic energy
                spline.seek  ( sqrt(r2) );
                spline.preval( ISs ); S = spline.y(); dS = spline.dy();
                spline.preval( ITs ); T = spline.y(); dT = spline.dy();
    ...
    void CoulombOrbPair( int nrho, int nV, double* rhos, Vec3d* rhoPs, Vec3d* rhoFs,  double* Vs, Vec3d* VPs, Vec3d* VFs ){
        for(int i=0; i<nV; i++){
            ...
            for(int j=0; j<nrho; j++){
                ...
                double x   =  r - i;
                double qj  = rhos [j];
                Spline_Hermite::valdval( x, C, dC, ICs[i], ICs[i+1], ICs[i+2], ICs[i+3] );  // coulomb integrals
            }
        }
    }
    ...
    void prepareIntegralTables(){
        ...
        /*
        integrateSK( nsamp, 0, nsampI, dsamp, Rcut, dsamp, Wfs,     Wfs,  ISs, ITs );  // Basis Overlap and Kinetic energy
        integrateS ( nsamp, 0, nsampI, dsamp, Rcut, dsamp, Wf2s,    Vfs,  ICs      );  // Coulomb betwween density functions
        integrateS ( nsamp, 0, nsampI, dsamp, Rcut, dsamp, PPs[0],  Wf2s, IrhoV[0] );  // Coulomb with ion pseudopotential
        */
    }
