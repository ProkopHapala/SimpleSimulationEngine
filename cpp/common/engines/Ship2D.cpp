...
void Ship2D::applyHydroForces( const Vec2d& watterSpeed ){
	Vec2d lvel;
	lvel.set_sub( watterSpeed, vel );
	//printf( " keel   ");
	keel  .assertAeroForce( *this, lvel, 1000.0 );  //   printf( " f keel (%3.3f,%3.3f) \n", force.x, force.y  );
	//printf( " rudder ");
...
void Ship2D::applySailForces( const Vec2d& windSpeed ){
    Vec2d lvel;
...
void Ship2D::testSail( int n, int nsub, double dt, double windSpeed, Vec2d * poss, Vec2d * vels, Vec2d * rots ){
    //pos.set(0.0);
    //vel.set(0.0);
    ...
    for( int i=0; i<n; i++ ){
        for ( int j=0; j<nsub; j++ ){
            clean_temp( );
            applyHydroForces( watter_speed );
            applySailForces ( wind_speed );
            move( dt );
        }
...
int Ship2D::convergeSail( int nmax, int nsub, double dt, double windSpeed, double vtol, double rtol, Vec2d& vel_conv, Vec2d& rot_conv ){
...
void Ship2D::evalPolar( int n, double dt, double vtol, double rtol, double * phi_rudder, double * phi_mast, double * wind_speed, Vec2d * vels, Vec2d * rots, bool reseting ){
    for( int i =0; i<n; i++ ){
        pos.set(0.0d);
        if( reseting ){
