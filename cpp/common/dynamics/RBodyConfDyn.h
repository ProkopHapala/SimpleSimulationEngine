...
class RBodyConfDyn{
    ...
    void forceFomConf( const Vec3d& pos, const Quat4d rot, const Vec3d& pos0, const Quat4d rot0, Vec3d& fpos, Quat4d& frot ){
        // --- position force
    ...
    void assembleConfForces(){
        ...
        Vec3d  pos   = *(Vec3d* )(optimizer.pos  );
        Quat4d rot   = *(Quat4d*)(optimizer.pos+3); 
        for(int i=0; i<nConfs; i++){
            RBodyPose* conf_i = confs+i;
            forceFomConf( pos, rot, conf_i->pos, conf_i->rot, fpos, frot );
        }
