...
class MechPIC2D{ public:
    ...
    */
    ...
    void evalCellThermodynamics(){
        ...
        for(int i=0; i<nctot; i++){
            double T0  = temperature[i];
            double N0  = moles    [i];
            double N1  = moles_new[i];
            // To Do : We must update temperature based on
            temperature[i] = T0 * pow( N1/N0, 2./3. );  // store updated temperature
            moles      [i] = N1;                        // store updated molar amount
    ...
    void particlesToCells(){
        ...
        for(int i=0; i<np; i++){
            ...
            double mx = 1-d.x;
            double my = 1-d.y;
            moles_new[ic      ] += nmol*mx *my;
            moles_new[ic+1    ] += nmol*d.x*my;
            moles_new[ic+io10 ] += nmol*d.y*mx;
            moles_new[ic+io11 ] += nmol*d.x*d.y;
    ...
    void moveMD( double dt ){
        ...
        for(int i=0;i<np; i++){
            ...
            double N00 = nmol*mx * my/moles[ic      ];
            double N01 = nmol*d.x* my/moles[ic+1    ];
            double N10 = nmol*d.y* mx/moles[ic+io10 ];
            double N11 = nmol*d.x*d.y/moles[ic+io11 ];
            int jc;
