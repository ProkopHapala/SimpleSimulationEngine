...
def parse_args():
    ...
    # Physics / mixing
    p.add_argument("--friction",        type=float, default=0.1,    help="Wall friction coefficient")
    p.add_argument("--viscosity",       type=float, default=0.1,    help="Artificial viscosity magnitude")
    ...
    p.add_argument("--inject-period",   type=float, default=0.020,  help="Injection/ignition period [s]")
    p.add_argument("--inject-start",    type=float, default=0.005,  help="First injection start time [s]")
    p.add_argument("--inject-duration", type=float, default=0.005,  help="Injection duration per cycle [s]")
    p.add_argument("--inject-point",    type=float, default=0.0,   help="Injection point (absolute x in m)")
    p.add_argument("--fuel-mass",       type=float, default=3e-4,   help="Total fuel mass per cycle [kg]")
    p.add_argument("--ignite-delay",    type=float, default=0.001,    help="Ignition delay after injection end within each cycle [s]")
    return p.parse_args()
...
def physics_step():
    global nodes_x, nodes_v, elem_species, elem_entropy, sim_time, last_ignite_cycle
    ...
    # Periodic cycle logic
    if sim_time >= args.relax_time and sim_time >= args.inject_start:
        phase = (sim_time - args.inject_start) % args.inject_period
        cycle_idx = int((sim_time - args.inject_start) // args.inject_period)
        
        # A. Injection
        if 0 <= phase < args.inject_duration:
            inj_x = args.inject_point - geom_x0
            idx = np.argmin(np.abs(centers - inj_x))
            elem_species[idx, 2] += (args.total_fuel_mass / args.inject_duration) * dt
            
        # B. Ignition
        ign_start = args.inject_duration + args.ignite_delay
        if ign_start <= phase < ign_start + dt*1.1 and cycle_idx != last_ignite_cycle:
            last_ignite_cycle = cycle_idx
            inj_x = args.inject_point - geom_x0
            chamber_mask = np.abs(centers - inj_x) < 0.05
            idxs = np.where(chamber_mask)[0]
            for i in idxs:
                burn = min(elem_species[i, 2], elem_species[i, 1] / O2_PER_FUEL)
                if burn > 1e-12:
                    elem_species[i, 2] -= burn
                    elem_species[i, 1] -= burn * O2_PER_FUEL
                    elem_species[i, 3] += burn * Y_PROD_CO2
                    elem_species[i, 4] += burn * Y_PROD_H2O
                    elem_entropy[i] += (burn * HEAT_OF_COMBUSTION / temps[i]) * 0.8
...
def update_plot(frame):
    ...
    # Compute time to next injection / ignition
    if sim_time >= args.inject_start:
        phase = (sim_time - args.inject_start) % args.inject_period
        t_to_inj = 0.0 if phase < args.inject_duration else (args.inject_period - phase)
        t_to_ign = max(0.0, args.inject_duration + args.ignite_delay - phase)
    else:
        t_to_inj = args.inject_start - sim_time
        t_to_ign = t_to_inj + args.inject_duration + args.ignite_delay
    ...
    ax_geo.set_title(f"t={sim_time*1000:.1f}ms | next_inj={t_to_inj*1000:.1f}ms | next_ign={t_to_ign*1000:.1f}ms")
