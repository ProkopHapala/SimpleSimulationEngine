...
Rigid body dynamics in 2D (`Body2D.h`) and 3D (`Body.h`), used in molecular dynamics (`MolecularWorld.h`), flight simulation (`AeroCraft.h`), tank simulation, and sailing. Python proof/derivation in `docs/RigidBodyDynamics/RigidBodyProperly.md` with companion `rigid_body_proof.py`. Applications span multiple game/sim apps.
...
- `AeroCraft.h` aerodynamic model not documented in topical audit — see `docs/AeroCombat/`
